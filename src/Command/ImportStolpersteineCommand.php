...
class ImportStolpersteineCommand extends Command
{
    protected $FIELD_MAP = [
        'PersonFamilyNameLabel' => 'familyName',
        'PersonGivenNameLabel' => 'givenName',
        'StolpersteinLabel' => 'disambiguatingDescription',
        'Person' => 'wikidata',
        'todesDatum' => 'deathDate',
        'Gedenkbuch' => 'wikidata:P7571',
    ];

    protected $DATAPROVIDER = 2; // TODO: come up with a table
    ...
    {
        $this
            ->setName('import:stolpersteine-wikidata')
            ->setDescription('Import Stolpersteine from Wikidata')
        ;
    }
    ...
    {
        $fname = $this->dataDir
               . '/Stolpersteine_Gedenkbuch_Familiename_A_17-09-2022.xlsx';
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $rowObj) {
                ...
                if (empty($row['wikidata'])) {
                    continue;
                }

                $row['wikidata'] = str_replace('http://www.wikidata.org/entity/', '', $row['wikidata']);
                if (!preg_match('/^Q\d+$/', $row['wikidata'])) {
                    continue;
                }

                $row['identifier'] = 'SSWD.Per.' . $row['wikidata'];

                // make sure we don't produce duplicates
                if (array_key_exists($row['identifier'], $identifiers)) {
                    continue;
                }
