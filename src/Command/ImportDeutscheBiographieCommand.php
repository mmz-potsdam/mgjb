...
// src/App/Command/ImportDeutscheBiographie.php
...
class ImportDeutscheBiographieCommand extends Command
{
    protected $FIELD_MAP = [
        'Name' => 'name',
        'GND ID' => 'gnd',
    ...
    protected $DATAPROVIDER = 7; // TODO: come up with a table
    ...
    {
        $this
            ->setName('import:deutsche-biographie')
            ->setDescription('Import Deutsche Biographie')
        ;
    }
    ...
    {
        $fname = $this->dataDir
               . '/20220917_DeutscheBiographie_A.xlsx';
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $rowObj) {
                ...
                if (!empty($row['gnd'])) {
                    $row['identifier'] = 'DBIO.Per.' . $row['gnd'];
                    $row['url'] = sprintf(' https://www.deutsche-biographie.de/%s.html#ndbcontent', $row['gnd']);
                }
                ...
                $person->disambiguatingDescription = 'Deutsche Biographie';
